 ...
 */
@Command(name = "javafmt", mixinStandardHelpOptions = true, version = "1.0", description = "Format Java w/JBang source files using EclipseJava formatters")
public class javafmt implements Callable<Integer> {
	...
	static class EclipseJavaFormatter implements JavaFormatter {
		...
		public EclipseJavaFormatter(String settingsName, Map<String, String> settings, boolean jbangFriendly) {
		...
		public String format(String content) throws Exception {
			return format(content, jbangFriendly ? JBangDirectiveHandler.identifyJavaRanges(content)
	...
	@Option(names = "--jbang-friendly", negatable = true, description = "Use JBang friendly formatter settings (protects JBang directives)", defaultValue = "true", fallbackValue = "true")
	...
	public Integer call() throws Exception {
		...
		try {
			...
			if (settingsFile != null) {
				...
				formatter = new EclipseJavaFormatter(settingsFile.toString(), settings, jbangFriendly);
			} else {
				formatter = new EclipseJavaFormatter("default", null, jbangFriendly);
			}
	...
	private static Map<String, String> loadSettingsFromResource(String resourceName) throws IOException {
		try (InputStream is = javafmt.class.getClassLoader().getResourceAsStream(resourceName)) {
			if (is == null) {
